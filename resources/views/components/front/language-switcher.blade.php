...
<div class="lang-switcher {{ $class }}" role="group" aria-label="{{ __('front.nav.select_lang') }}">
    ...
    <a href="{{ $buildUrl('ca') }}" class="lang-switcher-option" aria-pressed="{{ $current === 'ca' ? 'true' : 'false' }}" hreflang="ca">CA</a>
    <span class="lang-switcher-sep" aria-hidden="true">/</span>
    <a href="{{ $buildUrl('es') }}" class="lang-switcher-option" aria-pressed="{{ $current === 'es' ? 'true' : 'false' }}" hreflang="es">ES</a>
</div>
