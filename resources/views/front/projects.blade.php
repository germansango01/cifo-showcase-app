            ...
            <form class="filter-toolbar-inner" method="GET" action="{{ route('projects') }}">
                ...
                <div class="filter-select-wrap">
                    ...
                    <select id="filter-cycle" name="cycle" aria-label="{{ __('front.projects.filter_cycle_aria') }}">
                        ...
                        <option value="DAW" @selected(request('cycle')==='DAW' )>DAW — {{ __('front.projects.filter_cycle_daw') }}</option>
                        <option value="DAM" @selected(request('cycle')==='DAM' )>DAM — {{ __('front.projects.filter_cycle_dam') }}</option>
                        <option value="ASIR" @selected(request('cycle')==='ASIR' )>ASIR — {{ __('front.projects.filter_cycle_asir') }}</option>
                        <option value="SMX" @selected(request('cycle')==='SMX' )>SMX — {{ __('front.projects.filter_cycle_smx') }}</option>
                    </select>
                </div>

                {{-- Docente --}}
                <div class="filter-select-wrap">
                    <label for="filter-professor" class="sr-only">{{ __('front.projects.filter_teacher_aria') }}</label>
                    <select id="filter-professor" name="professor" aria-label="{{ __('front.projects.filter_teacher_aria') }}">
                ...
                {{-- Reset --}}
                <a href="{{ route('projects') }}" class="filter-reset" role="button" aria-label="{{ __('front.projects.filter_reset_aria') }}">
