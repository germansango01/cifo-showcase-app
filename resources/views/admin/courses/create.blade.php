    ...
    <x-admin.ui.card class="max-w-lg">
        <form method="POST" action="{{ route('courses.store') }}" novalidate x-data="{
            ...
            <x-admin.ui.select name="category_id" :label="__('admin.courses.category')" icon="icofont-folder" :options="$categoryOptions"
                :placeholder="__('admin.courses.category_placeholder')" :required="true" x-model="form.category_id" @change="form.validate('category_id')" />

            <x-admin.ui.input name="course_code" :label="__('admin.courses.code')" icon="icofont-tag" :placeholder="__('admin.courses.code_placeholder')"
                :required="true" x-model="form.course_code" @change="form.validate('course_code')" />

            <x-admin.ui.translatable-input name="name" :label="__('admin.courses.name')" icon="icofont-book-alt" :placeholder="__('admin.courses.name_placeholder')"
