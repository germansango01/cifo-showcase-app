<x-layouts.admin :title="'Editar · ' . $user->name">
    ...
    <x-admin.ui.card class="max-w-2xl">
        <form method="POST" action="{{ route('users.update', $user) }}" x-data="{
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <x-admin.ui.input name="password_confirmation" label="Confirmar nueva contraseña" type="password"
            ...
            <div class="mt-6 flex flex-wrap gap-x-6 gap-y-1 text-xs opacity-50">
                ...
                <span><i class="icofont-clock-time"></i> Actualizado:
                    {{ $user->updated_at->format('d/m/Y H:i') }}</span>
                @if ($user->email_verified_at)
                    <span><i class="icofont-check-circled text-success"></i> Email verificado:
