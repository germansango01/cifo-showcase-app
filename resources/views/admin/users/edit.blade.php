<x-layouts.app :title="'Editar · ' . $user->name">
    ...
    <x-admin.ui.card class="max-w-2xl">
        ...
        >
            ...
            {{-- Contraseña (opcional) --}}
            <div class="divider my-5 text-xs opacity-60">Cambio de contraseña (opcional)</div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <x-admin.ui.input
                    ...
                    help="Mínimo 8 caracteres. Déjalo vacío para mantener la actual."
                    ...
                    <fieldset class="fieldset">
                        ...
                        @if(auth()->user()->hasRole('Super Admin') && $user->hasRole('Super Admin') && auth()->id() === $user->id)
                            <x-admin.ui.alert type="warning" class="mt-2 mb-3">
                                No puedes quitarte el rol <strong>Super Admin</strong> a ti mismo.
                            </x-admin.ui.alert>
                        @endif
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
                            @foreach($roles as $role)
                                ...
                                @endphp
                                <label class="flex items-center gap-2 cursor-pointer select-none group {{ $isProtected ? 'opacity-60 pointer-events-none' : '' }}">
                                    <input
                                        type="checkbox"
                                        name="roles[]"
                                        value="{{ $role->name }}"
            ...
            <div class="mt-6 flex flex-wrap gap-x-6 gap-y-1 text-xs opacity-50">
                <span><i class="icofont-calendar"></i> Creado: {{ $user->created_at->format('d/m/Y H:i') }}</span>
                <span><i class="icofont-clock-time"></i> Actualizado: {{ $user->updated_at->format('d/m/Y H:i') }}</span>
                @if($user->email_verified_at)
                    <span><i class="icofont-check-circled text-success"></i> Email verificado: {{ $user->email_verified_at->format('d/m/Y') }}</span>
                @else
                    <span><i class="icofont-warning-alt text-warning"></i> Email no verificado</span>
                @endif
            </div>
