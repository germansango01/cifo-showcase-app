    ...
    <x-admin.ui.card class="max-w-lg">
        <form method="POST" action="{{ route('categories.update', $category) }}" novalidate
            ...
            <x-admin.ui.translatable-input name="name" :label="__('admin.categories.col_name')"
                icon="icofont-sub-listing" :required="true"
                :value-es="old('name.es', $category->getTranslation('name', 'es', false))"
                :value-ca="old('name.ca', $category->getTranslation('name', 'ca', false))"
                form-var="form" />
