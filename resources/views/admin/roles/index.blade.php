    ...
    <x-admin.ui.card>
        <x-admin.table.index :items="$roles" :columns="['Rol', 'Permisos', 'Usuarios', '']">
            ...
            <tr>
                ...
                <td class="text-right">
                    @if($role->name !== 'Super Admin')
                    <x-admin.table.actions :edit-url="route('roles.edit', $role)" :delete-url="route('roles.destroy', $role)" :delete-confirm="'¿Eliminar el rol «' . $role->name . '»? Los usuarios que lo tengan asignado lo perderán.'" edit-permission="roles.update" delete-permission="roles.delete" />
                    @else
